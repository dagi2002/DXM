...
// API shape from GET /analytics/vitals
interface VitalsApiResponse {
  lcp?:  { value: number; p50: number; p75: number; p95: number };
  fcp?:  { value: number; p50: number; p75: number; p95: number };
  cls?:  { value: number; p50: number; p75: number; p95: number };
  ttfb?: { value: number; p50: number; p75: number; p95: number };
  inp?:  { value: number; p50: number; p75: number; p95: number };
  fid?:  { value: number; p50: number; p75: number; p95: number };
}
...
function calcVitalStatus(name: string, value: number): VitalStatus {
  if (name === 'cls') {
...
export const PerformanceMetrics: React.FC = () => {
  const [vitals, setVitals] = useState<VitalsApiResponse | null>(null);

  useEffect(() => {
    fetchJson<VitalsApiResponse>('/analytics/vitals?period=7d')
      .then(data => setVitals(data))
      .catch(() => { /* API unavailable — display dashes */ });
  }, []);

  const performanceData: CoreWebVital[] = [
    {
      metric: 'First Contentful Paint',
      value: vitals?.fcp ? msToDisplay(vitals.fcp.value) : '1.2s',
      status: vitals?.fcp ? calcVitalStatus('fcp', vitals.fcp.value) : 'good',
      benchmark: '< 1.8s',
      trend: 'down',
      change: -8,
      p75: vitals?.fcp ? msToDisplay(vitals.fcp.p75) : '1.4s',
      p90: vitals?.fcp ? msToDisplay(vitals.fcp.p95) : '1.8s',
      description: 'Time until first text or image is painted',
    },
    {
      metric: 'Largest Contentful Paint',
      value: vitals?.lcp ? msToDisplay(vitals.lcp.value) : '2.1s',
      status: vitals?.lcp ? calcVitalStatus('lcp', vitals.lcp.value) : 'needs-improvement',
      benchmark: '< 2.5s',
      trend: 'up',
      change: 12,
      p75: vitals?.lcp ? msToDisplay(vitals.lcp.p75) : '2.8s',
      p90: vitals?.lcp ? msToDisplay(vitals.lcp.p95) : '3.2s',
      description: 'Time until largest content element is painted',
    },
    {
      metric: 'Cumulative Layout Shift',
      value: vitals?.cls ? vitals.cls.value.toFixed(3) : '0.08',
      status: vitals?.cls ? calcVitalStatus('cls', vitals.cls.value) : 'good',
      benchmark: '< 0.1',
      trend: 'down',
      change: -15,
      p75: vitals?.cls ? vitals.cls.p75.toFixed(3) : '0.12',
      p90: vitals?.cls ? vitals.cls.p95.toFixed(3) : '0.18',
      description: 'Visual stability of page during loading',
    },
    {
      metric: 'First Input Delay',
      value: vitals?.fid ? msToDisplay(vitals.fid.value) : '45ms',
      status: vitals?.fid ? calcVitalStatus('fid', vitals.fid.value) : 'good',
      benchmark: '< 100ms',
      trend: 'down',
      change: -22,
      p75: vitals?.fid ? msToDisplay(vitals.fid.p75) : '68ms',
      p90: vitals?.fid ? msToDisplay(vitals.fid.p95) : '95ms',
      description: 'Time from first user interaction to browser response',
    },
    {
      metric: 'Interaction to Next Paint',
      value: vitals?.inp ? msToDisplay(vitals.inp.value) : '180ms',
      status: vitals?.inp ? calcVitalStatus('inp', vitals.inp.value) : 'good',
      benchmark: '< 200ms',
      trend: 'stable',
      change: 2,
      p75: vitals?.inp ? msToDisplay(vitals.inp.p75) : '220ms',
      p90: vitals?.inp ? msToDisplay(vitals.inp.p95) : '280ms',
      description: 'Responsiveness of page to user interactions',
    },
    {
      metric: 'Time to First Byte',
      value: vitals?.ttfb ? msToDisplay(vitals.ttfb.value) : '320ms',
      status: vitals?.ttfb ? calcVitalStatus('ttfb', vitals.ttfb.value) : 'good',
      benchmark: '< 600ms',
      trend: 'down',
      change: -18,
      p75: vitals?.ttfb ? msToDisplay(vitals.ttfb.p75) : '450ms',
      p90: vitals?.ttfb ? msToDisplay(vitals.ttfb.p95) : '680ms',
          ...
          <div className="relative h-40 mb-4">
            <div className="absolute inset-0 flex items-end justify-between space-x-1">
              {Array.from({ length: 24 }, (_, index) => {
                const height = ((index * 17 + 40) % 80) + 20;
