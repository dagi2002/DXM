      ...
      <div className="bg-white border border-gray-200 rounded-lg p-5">
        ...
        <dl className="grid grid-cols-2 gap-4">
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
            <dt className="text-xs font-medium text-blue-600 uppercase tracking-wide">Total Clicks</dt>
            <dd className="text-2xl font-semibold text-blue-700">{isLoading ? '—' : totalClicks}</dd>
          </div>
        ...
        {activeType === 'hover' && (
          <div className="mt-4 rounded-lg bg-slate-50 border border-dashed border-slate-200 px-4 py-3 text-sm text-gray-600">
            Real-time hover capture is not yet enabled. Use click or scroll heatmaps to explore current recordings.
          </div>
        )}
      </div>
